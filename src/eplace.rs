...
pub fn eplace(prev: NLparams, m: usize) -> NLparams {
    ...
    let new: NLparams = NLparams {
        placement: placement.clone(), 
        a : a,
        alpha : inverse_lipschitz_constant(&ref_placement, &prev.placement, &grad_f_k,prev.grad_f_k),
...
pub fn calc_cell_fields( placement: &Array2<f64>, m: usize) -> CellElectricFields {
    ...
    let cell_fields_x = placement.map_axis(Axis(1), |x| dct::apply_bins_to_cell(&x, &elec_field_x, m));
    let cell_fields_y = placement.map_axis(Axis(1), |x| dct::apply_bins_to_cell(&x, &elec_field_y, m));
...
fn inverse_lipschitz_constant(placement: &Array2<f64> , prev_placement: &Array2<f64>, grad_f_k: &Array1<f64> , prev_grad_f_k:Array1<f64>) -> f64 {
...
fn calc_density_overflow(placement: &Array2<f64>, m:usize ) -> f64 {
    ...
    let numerator = bin_density.map( |bin_density| 
        (bin_density-TARGET_DENSITY).max(0.) * grid_area  ).sum(); 
...
pub fn calc_gamma(density_overflow: f64) -> f64 {
    8. * BIN_W * (10 as f64).powf( K * density_overflow + B)
}

pub fn calc_grad_f_k (wl_gradient: &Array1<f64>, lambda: f64, fields: CellElectricFields ) -> Array1<f64> {
